...
public class MetricRegistry implements MetricSet {
     ...
     */
    public void registerAll(final MetricSet metrics)
            throws IllegalArgumentException {
    ...
    public SortedMap<String, Gauge> getGauges(final MetricFilter filter) {
        return getMetrics(MetricBuilder.GAUGES, filter);
    }
    ...
    public SortedMap<String, Counter> getCounters(final MetricFilter filter) {
        return getMetrics(MetricBuilder.COUNTERS, filter);
    }
    ...
    public SortedMap<String, Histogram> getHistograms(final MetricFilter filter) {
        return getMetrics(MetricBuilder.HISTOGRAMS, filter);
    }
    ...
    public SortedMap<String, Meter> getMeters(final MetricFilter filter) {
        return getMetrics(MetricBuilder.METERS, filter);
    }
    ...
    public SortedMap<String, Timer> getTimers(final MetricFilter filter) {
        return getMetrics(MetricBuilder.TIMERS, filter);
    }

    @SuppressWarnings("unchecked")
    private <T extends Metric> T getOrAdd(final String name,
            final MetricBuilder<T> builder) {
    ...
    private <T extends Metric> SortedMap<String, T> getMetrics(
            final MetricBuilder<T> klass, final MetricFilter filter) {
    ...
    private void notifyListenerOfAddedMetric(
            final MetricRegistryListener listener, final Metric metric,
            final String name) {
    ...
    private void notifyListenerOfRemovedMetric(final String name,
            final Metric metric, final MetricRegistryListener listener) {
        if (metric instanceof Gauge) {
    ...
    private void registerAll(final String prefix, final MetricSet metrics)
            throws IllegalArgumentException {
        for (final Map.Entry<String, Metric> entry : metrics.getMetrics()
                .entrySet()) {
