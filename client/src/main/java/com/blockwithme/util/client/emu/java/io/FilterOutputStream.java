...
public class FilterOutputStream extends OutputStream {
    ...
    /**
     * Returns the current wrapped output stream.
     * WRANING: Not part of real FilterOutputStream.
     */
     ...
     * WRANING: Not part of real FilterOutputStream.
     */
    public void setWrapped(OutputStream newOut) {
        out = newOut;
    }
    ...
    public void write(byte buffer[], int offset, int length) throws IOException {
        ...
        if (offset > buffer.length || offset < 0) {
            ...
            throw new ArrayIndexOutOfBoundsException("Bad offset: "+offset);
        }
        if (length < 0 || length > buffer.length - offset) {
            // luni.18=Length out of bounds \: {0}
            throw new ArrayIndexOutOfBoundsException("Bad length: "+length);
        }
        doValidatedWrite(buffer, offset, length);
    }

    protected void doValidatedWrite(byte buffer[], int offset, int length) throws IOException {
        if (out != null) {
