...
public class ExponentiallyDecayingReservoir implements Reservoir {
    ...
    private final TreeMap<Double, Long> values;
    ...
    public ExponentiallyDecayingReservoir(final int size, final double alpha,
            final Clock clock) {
        this.values = new TreeMap<Double, Long>();
    ...
    public void update(final long value, final long timestamp) {
        ...
        try {
            ...
            if (newCount <= size) {
                ...
                if (first < priority && putIfAbsent(priority, value) == null) {
    ...
    private Long putIfAbsent(final Double priority, final Long value) {
        final Long result = values.get(priority);
        if (result == null) {
            values.put(priority, value);
        }
        return result;
